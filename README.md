...
予約リンクはSlackによるShift-JIS URLの二重エンコードを防ぐため、検証済みのTinyURLを使用します。対応する日付・列車は `sunrise_booking_links.json` に保存し、転送先が `search_url()` と完全一致することを確認しています。監視日を変更するときは短縮リンクも作成・検証してください。未登録の日付では誤った予約先を送らずエラーにします。
