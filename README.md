...
2026年9月24日 岡山22:34発→東京のサンライズ瀬戸・出雲を、既存の15分間隔の定期実行から確認します。
通知先は専用チャンネル `C0C2LSTJD1T`（`SLACK_CHANNEL_SUNRISE`）。ピックルボールの通知先とは独立しています。
空席が出たときに加え、前回空いていた列車・設備区分が空席なしになったときも「埋まりました」と通知します。
変化がなければ通知せず、照会失敗・メンテナンスは満席扱いにしません。Slack送信成功後に状態を保存します。
B寝台は個室タイプの総合表示なので、個別の部屋タイプ単位ではなく検索画面の設備区分単位の変化です。
`sunrise.yml` の手動実行で `notify=true` と `status_update=true` を指定すると、確認できた現在の状況を専用チャンネルに投稿します。


サンライズ監視でe5489が明示的に「混雑中」を返した場合は、3回の試行後も続いていても定期処理を失敗扱いにせず、次の約15分後に再確認します。空席状態は保持します。最初の確認失敗から1時間以上継続したことを次の実行で検知すると、専用Slackに1回通知し、照会が成功したときに復旧を1回通知します。短時間で復旧した場合は障害・復旧とも通知しません。障害の開始時刻と通知済みフラグは `sunrise_health.json` に永続化します。ドライランは障害状態を変更しません。未知の画面・タイムアウト・プログラム異常・Slack送信失敗は引き続き失敗扱いです。

予約リンクはSlackによるShift-JIS URLの二重エンコードを防ぐため、管理下のGitHub Pages中継ページを使用します。
ソース: https://github.com/takuyamotomura-2026/sunrise-booking-links
日付・列車別の固定ページであり、外部短縮サービスには依存しません。監視日を変更するときは、中継ページと `sunrise_booking_links.json` を追加し、公開ページの転送先が `search_url()` と一致することを検証してください。未登録の日付では誤った予約先を送らずエラーにします。

2026-09-18: 予約完了のためサンライズの自動監視を停止。pickleball.yml から sunrise 呼び出しを外し、sunrise.yml を無効化しています。ピックルボールの定期監視と予約リンク用のGitHub Pagesは継続します。再開時は監視日・リンク・前回状態を確認し、ワークフローの再有効化と呼び出しの復元を行ってください。
